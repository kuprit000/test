...
limitawal = 10
memberlimit = 0
cr = '*FOLLOW IG:ITZ.DAPAA*'

/******** OWNER NUMBER**********/
const ownerNumber = ["6285758779973@s.whatsapp.net"] 
const pacarNumber = ["6283197241008@s.whatsapp.net"]
/************************************/
...
itsmeiky.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Hallo* 👋 @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}*\nSemoga betah ya di sini 😅\nJangan lupa baca deskripsi @${num.split('@')[0]} 😅`
	...
	itsmeiky.on('message-new', async (iky) => {
		...
		case 'transfer':
				...
				if (!q.includes('|')) return  reply(ind.wrongf())
                		...
                		addKoinUser('6285758779973@s.whatsapp.net', fee)
...
//If INFO DEVELOPER not in change, its okey!!
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    shiz = ` ◪ *INFO DEVELOPER*
  ❏ Nama: Ａｒｄｅｓｔｏ
  ❏ Wa: wa.me/6285758779973
  ❏ Ig: https://instagram.com/itz.dapaa
  ----------------------------------
◪ *INFO SHIZUKA*
  ❏ Nama: SHIZUKA 
  ❏ Web: ------------------
...
◪ *POWERED BY Ａｒｄｅｓｔｏ*`
		...
		case 'lpr':
					...
					itsmeiky.sendMessage('6285758779973@s.whatsapp.net', options, text, {quoted: iky})
		...
		case 'hargapremium':
					...
					break
		case 'buypremium':
					itsmeiky.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/6285758779973',MessageType.text, { quoted: iky} )
					break
