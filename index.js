...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Halo @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}* patuhi rules nya ya`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const AzizAE = ["6285758779973@s.whatsapp.net"]
		...
		case 'daftar':
         ...
         case 'lirik':
                ...
                if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('6285758779973@s.whatsapp.net', options, text, {quoted: mek})
		    ...
		    case 'map':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧 BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧]','red'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
